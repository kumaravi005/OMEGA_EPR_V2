...
final enquiryControllerProvider = Provider<EnquiryController>((ref) => EnquiryController(ref));
...
class EnquiryController {
  ...
  Future<void> submitEnquiry({
    ...
    try {
      ...
      throw const EnquiryFailure('Could not submit your enquiry. Please try again.');
    }
  }

  Future<void> updateStatus(Enquiry existing, EnquiryStatus status) async {
    try {
      await _ref.read(enquiryRepositoryProvider).updateFields(existing.enquiryId, {
        'status': status.name,
        'updatedAt': Timestamp.now(),
      });
    } catch (_) {
      throw const EnquiryFailure('Could not update the enquiry. Please try again.');
    }
  }

  /// Submitted by an unauthenticated visitor - no sign-in required.
  Future<void> submitCallbackRequest({required String name, required String phone, required String? message}) async {
    try {
      final now = DateTime.now();
      await _ref
      ...
      throw const EnquiryFailure('Could not submit your request. Please try again.');
    }
  }

  Future<void> updateCallbackStatus(CallbackRequest existing, CallbackStatus status) async {
    try {
      await _ref.read(callbackRequestRepositoryProvider).updateFields(existing.requestId, {
        'status': status.name,
        'updatedAt': Timestamp.now(),
      });
    } catch (_) {
      throw const EnquiryFailure('Could not update the request. Please try again.');
    }
  }
