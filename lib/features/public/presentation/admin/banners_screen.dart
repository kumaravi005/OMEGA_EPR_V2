    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: bannersAsync.when(
          ...
          error: (error, stackTrace) => ErrorView(message: 'Could not load banners.\n$error'),
          data: (banners) {
            if (banners.isEmpty) return const EmptyView(message: 'No banners yet.');
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Card(
      child: ListTile(
        ...
        subtitle: Text(banner.isLive(DateTime.now()) ? 'Live now' : 'Not currently live'),
        ...
        trailing: Switch(
          ...
          onChanged: (value) async {
            ...
            try {
              await ref.read(publicContentControllerProvider).setBannerActive(banner, value);
...
void _showBannerForm(BuildContext context, {BannerItem? existing}) {
  showDialog<void>(context: context, builder: (context) => _BannerFormDialog(existing: existing));
}
...
class _BannerFormDialogState extends ConsumerState<_BannerFormDialog> {
  ...
  late final _ctaTextController = TextEditingController(text: widget.existing?.ctaText ?? '');
  late final _ctaUrlController = TextEditingController(text: widget.existing?.ctaUrl ?? '');
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: AppSpacing.sm),
              AppTextField(controller: _ctaTextController, label: 'CTA button text (optional)', enabled: !_isSubmitting),
              const SizedBox(height: AppSpacing.sm),
              AppTextField(controller: _ctaUrlController, label: 'CTA link URL (optional)', enabled: !_isSubmitting),
              ListTile(
                contentPadding: EdgeInsets.zero,
                title: Text(_displayFrom == null ? 'Display from (optional)' : 'From ${dateKey(_displayFrom!)}'),
                trailing: const Icon(Icons.calendar_today_outlined),
                onTap: _isSubmitting ? null : () => _pickDate(isFrom: true),
              ),
              ListTile(
                contentPadding: EdgeInsets.zero,
                title: Text(_displayUntil == null ? 'Display until (optional)' : 'Until ${dateKey(_displayUntil!)}'),
                trailing: const Icon(Icons.calendar_today_outlined),
