...
class PdfReportBuilder {
  ...
  Future<Uint8List> build(ExportDataset dataset) async {
    ...
    final pageFormat = (dataset.isLandscape ? PdfPageFormat.a4.landscape : PdfPageFormat.a4).copyWith(
      marginLeft: _margin,
      marginRight: _margin,
      marginTop: _margin + 4,
      marginBottom: _margin + 4,
    );
    final contentWidth = pageFormat.width - pageFormat.marginLeft - pageFormat.marginRight;

    final branding = dataset.branding;
    final logo = branding?.header.logoUrl == null ? null : await _fetchLogo(branding!.header.logoUrl!);
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        footer: (context) =>
            branding == null ? _plainFooter(context) : _brandingFooter(branding.footer, context),
        build: (context) => [
  ...
  pw.Widget _brandingHeader(ReportHeaderBranding header, pw.MemoryImage? logo, double contentWidth) {
    ...
    final textLines = <pw.Widget>[
      if (header.instituteName != null)
        pw.Text(header.instituteName!, style: pw.TextStyle(fontSize: 15, fontWeight: pw.FontWeight.bold)),
      if (header.tagline != null)
        pw.Padding(
          padding: const pw.EdgeInsets.only(top: 1),
          child: pw.Text(header.tagline!, style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey700)),
        ),
      if (header.address != null)
        pw.Padding(padding: const pw.EdgeInsets.only(top: 1), child: pw.Text(header.address!, style: const pw.TextStyle(fontSize: 8))),
      if (header.contact != null)
        pw.Padding(padding: const pw.EdgeInsets.only(top: 1), child: pw.Text(header.contact!, style: const pw.TextStyle(fontSize: 8))),
      if (header.otherText != null)
        pw.Padding(padding: const pw.EdgeInsets.only(top: 1), child: pw.Text(header.otherText!, style: const pw.TextStyle(fontSize: 8))),
    ];
    ...
    if (logo != null) {
      ...
      final logoWidth = (placement.widthFraction * contentWidth).clamp(16.0, contentWidth);
      final logoHeight = logoWidth * (sourceHeight / sourceWidth);
      final left = (placement.xFraction * contentWidth).clamp(0.0, contentWidth - logoWidth);
      final top = (placement.yFraction * _headerHeight).clamp(0.0, (_headerHeight - logoHeight).clamp(0.0, _headerHeight));
    ...
    final textWidget = pw.Positioned(
      ...
      child: pw.SizedBox(
        ...
        child: pw.Column(
          crossAxisAlignment: logoOnLeftHalf ? pw.CrossAxisAlignment.end : pw.CrossAxisAlignment.start,
          children: textLines,
        ),
      ),
    );
    ...
    return pw.SizedBox(
      ...
      width: contentWidth,
      child: pw.Stack(children: [?logoWidget, if (textLines.isNotEmpty) textWidget]),
    );
  }
  ...
  pw.Widget _brandingFooter(ReportFooterBranding footer, pw.Context context) {
    final leftChildren = <pw.Widget>[
      if (footer.showSignature) ...[
        pw.Container(width: 130, height: 0.6, color: PdfColors.grey600, margin: const pw.EdgeInsets.only(bottom: 2)),
        pw.Text(footer.signatureLabel ?? 'Authorized Signatory', style: const pw.TextStyle(fontSize: 8)),
      ],
    ];

    final rightChildren = <pw.Widget>[
      if (footer.showDate) pw.Text('Date: ${_todayLabel()}', style: const pw.TextStyle(fontSize: 8)),
      if (footer.contactText != null) pw.Text(footer.contactText!, style: const pw.TextStyle(fontSize: 8)),
    ...
    return pw.Column(
      children: [
        ...
        pw.Row(
          ...
          children: [
            pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: leftChildren),
            if (footer.footerText != null)
              pw.Expanded(child: pw.Center(child: pw.Text(footer.footerText!, style: const pw.TextStyle(fontSize: 8)))),
            pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.end, children: rightChildren),
          ],
        ),
      ],
    );
